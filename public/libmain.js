...
var Module = (() => {
  var _scriptDir = chrome.runtime.getURL("/");
...
var wasmBinaryFile;
if (Module['locateFile']) {
  wasmBinaryFile = 'libmain.wasm';
  if (!isDataURI(wasmBinaryFile)) {
    wasmBinaryFile = locateFile(wasmBinaryFile);
  }
} else {
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  wasmBinaryFile = chrome.runtime.getURL('libmain.wasm');
}
...
// end include: preamble.js
  ...
  var PThread = {unusedWorkers:[],runningWorkers:[],tlsInitFunctions:[],pthreads:{},nextWorkerID:1,debugInit:function() {
        ...
        if (!Module['locateFile']) {
          worker = new Worker(chrome.runtime.getURL('libmain.worker.js'));
        } else {
  ...
  var _emscripten_get_now;_emscripten_get_now = () => performance.timeOrigin + performance.now();
  ;
